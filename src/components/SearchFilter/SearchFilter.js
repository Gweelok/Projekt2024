...
const SearchFilter = ({
  ...
  const renderItem = ({ item, index }) => {
    const itemName =
      item.brandName || item.productName || item.categoryName || item.modelName;
      ...
      return (
        <>
          ...
          >
            <Text
              style={[
                dropdownStyles.dropdownFilterTextHome,
                { color: Primarycolor1 },
              ]}
            >
              {input}
            </Text>
            <Text style={dropdownStyles.dropdownFilterTextHome}>
              {splittedItemName}
            </Text>
          </Text>
  ...
  const renderError = () => (
    <Text style={dropdownStyles.dropdownErrorText}>
      {t("SearchField.notMatchingProduct", currentLanguage)}
    </Text>
  );

  return (
    <View style={dropdownStyles.dropdownSearchFieldListContainer}>
      {isLoading ? (
        renderLoading()
      ) : error ? (
        renderError()
