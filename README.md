...
## Start (every system)
To start off with, it doesn’t matter what system you have, all the first steps are exactly the same, which is to say...
...
C:\MobileApps\
```
Open up your preferred Shell, if you don’t have any or don’t know what that is Node.js should have installed PowerShell. In the shell, that should look like a terminal, write:
```
...
npx expo start
```
And pray with all your might to your lord and savior Richard “Milos” Stallman that it worked. That should be it on the computer side, there’ll be a last section for how to run the application on your phone.
...
```
Remember to go into the folder where the app is, and then to start the system that’ll let you run the mobile app, that run the command:
```
npm install
npx expo start
```
...
Then for fun and to test if it works, add something to "App.js" maybe a comment with "// "or something, then in Source Control type something in the message box, usually what you've done, then press the "+" on the files you've changed and then press the tick to commit.
...
On mac when running application (using expo start), you might receive the error: 
```
